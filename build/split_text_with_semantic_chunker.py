...
def read_data(tx):
    ...
    """
    result = tx.run(
        "MATCH (n:News WHERE n.body IS NOT NULL) RETURN n.id, n.body, elementId(n) As elementId"
    )
    return [
        (record["n.id"], record["n.body"], record["elementId"]) for record in result
    ]


def write_data(tx, split_id, split_text, split_source):
    """
    ...
    tx.run(
        ...
        MERGE (n)<-[:CHILD_OF]-(ct:SplitText {split_id: $split_id, split_text: $split_text, split_source: $split_source})
        """,
        split_id=split_id,
...
for result in results:
    ...
    for index, text in enumerate(texts):
        ...
        try:
            ...
            driver.verify_connectivity()
            with driver.session() as session:
                session.execute_write(write_data, split_id, split_text, split_source)
